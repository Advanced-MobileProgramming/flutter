...
Future<void> removeBooksFromFirebase(List<String> booksToRemove) async {
  for (String bookId in booksToRemove) {
    ...
    // bookReports에서 해당 책 관련 데이터 삭제
    final DatabaseReference reportRef = FirebaseDatabase.instance.ref("bookReports/${widget.userId}/$bookId");
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Column(
                      children: [
                        ...
                        if (isEditing) // 편집 모드
                          Checkbox(
                            value: filteredBooks[index]["selected"] ?? false,
                            onChanged: (bool? value) {
                              setState(() {
                                filteredBooks[index]["selected"] = value ?? false;
                              });
                            },
                          ),
                      ],
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Column(
                      ...
                      children: [
                        Expanded(
                          child: GestureDetector(
                            onTap: () {
                              ...
                              if (!isEditing) {  // isEditing이 false일 때만 BookDetail로 이동
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                    builder: (context) => BookDetail(
                        ...
                        SizedBox(height: 8.0),
                        if (isEditing) // 편집 모드일 때만 체크박스 추가
                          Checkbox(
                            value: filteredBooks[index]["selected"] ?? false,
                            onChanged: (bool? value) {
                              setState(() {
                                ...
                                if (!isEditing) {
                                  ...
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) => BookDetail(
                                        userId: widget.userId,
                                        bookId: int.tryParse(filteredBooks[index]["id"] ?? '0') ?? 0, 
                                        nickname: widget.nickname,
                                      ),
                                    ),
                                  );
